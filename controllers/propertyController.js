...
exports.addNewProperty = async (req, res) => {
    ...
    try {
        ...
        if (localityResult.rows.length === 0) {
            const insertLocalityResult = await pool.query(
                'INSERT INTO localities (name) VALUES ($1) RETURNING id',
                [locality]
            );
            localityId = insertLocalityResult.rows[0].id;
        } else {
...
exports.deletePropertyRecord = async (req, res) => {
    ...
    try {
        const propertyResult = await pool.query('DELETE FROM properties WHERE id = $1 RETURNING *', [property_id]);
...
exports.fetchAllLocalities = async (req, res) => {
    try {
        const localitiesResult = await pool.query('SELECT * FROM localities');
        res.json(localitiesResult.rows);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
};
